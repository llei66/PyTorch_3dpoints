...
class GeoData_crop(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # import pdb
        # pdb.set_trace()
        # data = Data(pos=torch.tensor(current_points[:, :3]), x=torch.tensor(current_points[:, 3]).reshape(-1, 1),)
        # data = Data(pos=torch.tensor(current_points), x=None,)
        # data = Data(pos=torch.tensor(current_points).float(), x=torch.tensor(current_points[:, 2]).reshape(-1, 1),)
        data = Data(pos=torch.tensor(current_points).float())
        data.y = torch.tensor(current_labels).long()
        # data.y = current_labels
        # current_points = current_points + current_labels
        # return np.hstack((current_points, current_labels))
...
class GeoData_cropDataset(BaseDataset):
    ...
    def __init__(self, dataset_opt):
        ...
        #data_train_root = '/home/dmn774/Deepcrop/data/geodata/test_whole_one/train'
        #data_val_root = '/home/dmn774/Deepcrop/data/geodata/test_whole_one/train'
        #root_path = '/home/dmn774/Deepcrop/data/geodata/test_whole_one/'

        #root_path = '/home/dmn774/Deepcrop/data/geodata/20210430_train_test_val_set_whole_class/'
        root_path = '/home/dmn774/Deepcrop/data/geodata/202105_train_test_val_set_whole_class/'
        #root_path = '/home/dmn774/Deepcrop/data/geodata/202103_train_test_val_set_whole_class/'
        data_train_root = root_path + "train"
        data_val_root = root_path + "validate"
        data_test_root = root_path + "test"

        num_point, block_size, sample_rate = 10000, 30.0, 1

        self.train_dataset = GeoData_crop(split='train',
                                            data_root=data_train_root,
                                            num_point=num_point,
                                            block_size=block_size,
                                            sample_rate=sample_rate,
                                            transform=None)

        self.val_dataset = GeoData_crop(split='val',
        ...
        self.test_dataset = GeoData_crop(split='test',
