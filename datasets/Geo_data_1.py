...
class S3DISDataset(PointCloudDataset):
    ...
    def __init__(self, config, data_set, set='training', use_potentials=True, load_data=True):
        """
        ...
        # Dataset folder
        # self.path = '/data/REASEARCH/DEEPCROP/PointCloudData/clear_data/202102_train_test_val_set_whole_class_sub/'
        # self.path = '/data/REASEARCH/DEEPCROP/PointCloudData/clear_data/202102_train_test_val_set_whole_class_single/'
        self.path = data_set
        ...
        self.train_path = 'precess_kp_ply'
        ...
        self.cloud_names_split = ['train', 'test', 'validate']
        self.cloud_names = []
        ...
        elif self.set in ['validating', 'testing', 'ERF']:
        ...
        # geo data
        if self.set == 'training':
            train_file_path = join(ply_path, 'train')
            original_path = join(self.path, 'train')
            for room_name_ply in listdir(original_path):
                self.files +=[join(train_file_path, room_name_ply + '.ply')]
                self.cloud_names =[join(train_file_path, room_name_ply)]
        elif self.set == 'validating':
            validate_file_path = join(ply_path, 'validate')
            original_path = join(self.path, 'validate')

            for room_name_ply in listdir(original_path):
                self.files += [join(validate_file_path, room_name_ply + '.ply')]
                self.cloud_names = self.files

        elif self.set == 'testing':
            test_file_path = join(ply_path, 'test')
            original_path = join(self.path, 'test')

            for room_name_ply in listdir(original_path):
                self.files += [join(test_file_path, room_name_ply+ '.ply')]
                self.cloud_names = self.files
    ...
    def prepare_S3DIS_ply(self):
        ...
        for cloud_names_split in self.cloud_names_split:

            split_ply_path = join(ply_path, cloud_names_split)
            if not exists(split_ply_path):
                makedirs(split_ply_path)
            # Pass if the cloud has already been computed
            room_split = join(self.path, cloud_names_split)
            for room_name in listdir(room_split):
                room_path = join(room_split , room_name)

                cloud_file = join(split_ply_path, room_name + '.ply')
                print(cloud_file)
                ...
                cloud_colors = np.zeros((points.shape[0], 3), dtype=np.uint8)
                ...
                # cloud_colors = np.vstack((cloud_colors, cloud_colors.astype(np.uint8)))
                ...
                write_ply(cloud_file,
                          (points.astype(np.float32), cloud_colors.astype(np.uint8), labels),
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            KDTree_file = join(tree_path, '{:s}_KDTree.pkl'.format(cloud_name))
            sub_ply_file = join(tree_path, cloud_name)
        ...
        if self.use_potentials:
            ...
            for i, file_path in enumerate(self.files):
                ...
                # coarse_KDTree_file = join(tree_path, cloud_name)
        ...
        if self.set in ['validation', 'test']:
            ...
            for i, file_path in enumerate(self.files):
                ...
                proj_file = join(tree_path, cloud_name)
