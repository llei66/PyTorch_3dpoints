...
if __name__ == '__main__':
    file_path = '/data/REASEARCH/DEEPCROP/code/PYcode/point++_test/Pointnet_Pointnet2_pytorch/data/shapenetcore_partanno_segmentation_benchmark_v0'
#d = PartDataset(root = file_path, class_choice = ['Airplane'], split='test')
    d = PartDataset(root = file_path, split='test')
    print(len(d))
    import time
    tic = time.time()
    for i in range(100):
...
#print np.max(seg), np.min(seg)
    ...
    d = PartDataset(root = file_path, classification = True)
    print(len(d))
    ps, cls = d[0]
